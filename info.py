...
BOT_TOKEN = environ.get('BOT_TOKEN', "7775236007:AAHp-C6V0DN8n_UEvxNlF3_rk4YttzU6Kzw")
...
CODE = (environ.get('CODE', 'https://envs.sh/vTG.jpg'))
...
ADMINS = [int(admin) if id_pattern.search(admin) else admin for admin in environ.get('ADMINS', '7211073530 1735152469 1119579816 1632095203').split()]
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1002382270525').split()] #Channel id for auto indexing ( make sure bot is admin )
auth_users = [int(user) if id_pattern.search(user) else user for user in environ.get('AUTH_USERS', '').split()]
AUTH_USERS = (auth_users + ADMINS) if auth_users else []
PREMIUM_USER = [int(user) if id_pattern.search(user) else user for user in environ.get('PREMIUM_USER', '').split()]
auth_channel = environ.get('AUTH_CHANNEL', '-1002469144015') #Channel / Group Id for force sub ( make sure bot is admin )
auth_grp = environ.get('AUTH_GROUP')
AUTH_CHANNEL = int(auth_channel) if auth_channel and id_pattern.search(auth_channel) else None
AUTH_GROUPS = [int(ch) for ch in auth_grp.split()] if auth_grp else None
support_chat_id = environ.get('SUPPORT_CHAT_ID', '-1001448455100') # support group id ( make sure bot is admin )
reqst_channel = environ.get('REQST_CHANNEL_ID', '-1002258469523') # request channel id ( make sure bot is admin )
...
DATABASE_URI = environ.get('DATABASE_URI', "mongodb+srv://AjDonna3211:AjDonna3211@cluster0.xnkraij.mongodb.net/?retryWrites=true&w=majority")
DATABASE_NAME = environ.get('DATABASE_NAME', "techvjclonefilterbot")
COLLECTION_NAME = environ.get('COLLECTION_NAME', 'techvjclonefilterbot')

# Verify/token system
VERIFY = bool(environ.get('VERIFY', False)) # Verification On ( True ) / Off ( False )
# HOWTOVERIFY = environ.get('HOWTOVERIFY', url='https://t.me/tutorial_bash/16') 
...
GRP_LNK = environ.get('GRP_LNK', 'https://t.me/anime_bash_chat')
CHNL_LNK = environ.get('CHNL_LNK', 'https://t.me/anime_bash')
TUTORIAL = environ.get('TUTORIAL', 'https://t.me/tutorial_bash/16') # Tutorial video link for opening shortlink website 
IS_TUTORIAL = bool(environ.get('IS_TUTORIAL', True))
MSG_ALRT = environ.get('MSG_ALRT', 'ᴍᴀɪɴᴛᴀɪɴᴇᴅ ʙʏ : AjDonna')
LOG_CHANNEL = int(environ.get('LOG_CHANNEL', '-1002373327806')) #Log channel id ( make sure bot is admin )
SUPPORT_CHAT = environ.get('SUPPORT_CHAT', 'https://t.me/anime_bash_chat') #Support group link ( make sure bot is admin )
